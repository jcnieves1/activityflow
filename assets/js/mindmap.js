...
(function () {
  ...
  const STATUS_COLORS = {
    backlog: '#adb5bd', planned: '#0d6efd', ready: '#0dcaf0', in_progress: '#0d6efd',
    blocked: '#dc3545', waiting: '#ffc107', completed: '#198754', cancelled: '#6c757d',
  };
  function colorForStatus(slug) { return STATUS_COLORS[slug] || '#198754'; }
  ...
  function buildGraph(data) {
    ...
    data.releases.forEach((r) => {
      nodes.push({
        ...
        color: { background: '#6f42c1', border: '#5a32a3' }, font: { color: '#fff' },
      });
    });
    if (data.has_no_release_bucket) {
      nodes.push({
        id: 'r-none', label: i18n.noRelease || 'No release', level: 0, shape: 'box',
        color: { background: '#adb5bd', border: '#868e96' }, font: { color: '#fff' },
      });
    }
    ...
    data.projects.forEach((p) => {
      nodes.push({
        ...
        color: { background: '#0d6efd', border: '#0a58ca' }, font: { color: '#fff' },
      });
    ...
    data.tasks.forEach((t) => {
      nodes.push({
        ...
        color: { background: colorForStatus(t.status), border: '#00000033' },
        font: { color: '#fff' },
      });
    ...
    data.people.forEach((person) => {
      nodes.push({
        ...
        color: { background: '#fd7e14', border: '#c96208' }, font: { color: '#000' },
      });
    });
    data.tasks.forEach((t) => {
    ...
    const options = {
      layout: { hierarchical: { direction: 'UD', sortMethod: 'directed', levelSeparation: 140, nodeSpacing: 160 } },
      physics: false,
